...
export const beVietnamProVariantClassName: Partial<Record<TypographyVariant, string>> = {
  // button: 'text-center uppercase',
};

export function resolveBeVietnamProVariantMetrics({
  variant,
  weight,
  fontStyle = 'normal',
  responsiveType,
...
export function resolveBeVietnamProSizeMetrics({
  size,
  weight = 'Regular',
  fontStyle = 'normal',
  responsiveType,
  ...
  return {
    fontFamily: getFontFamily({ fontFamily: 'BeVietnamPro', fontStyle, weight }),
    fontSize: responsiveFontSize(getFontSize(size), scaleType),
  };
}

export function getBeVietnamProThemeMetrics(variant: TypographyVariant): FontMetrics {
  return resolveBeVietnamProVariantMetrics({ variant });
}
